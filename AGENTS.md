...
A social DJ web app inspired by Turntable.fm. Users hang out in virtual rooms, take turns DJing from a shared queue, and everyone listens together in sync via YouTube.

## Development

```bash
npm run dev    # Start with --watch (auto-restart on changes), port 3005
